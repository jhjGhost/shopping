...
window.onscroll = function(){
	...
	//当滚动距离大于自身距离顶部的距离时时执行下面的东西
	if(t>=realTop){
		$("details").style.position="fixed";
		$("details").style.top="0";
		$("details").style.zIndex="9999";
	}else{
		//当滚动距离小于自身距离顶部的距离时时，恢复原状
...
toRight.onclick=function(){
	var time=setInterval(function(){
		m++;
		document.querySelector(".rIcons ul").display.left="-"+m+"%";
		if(m==(n*100)){
			clearInterval(time);
			n++;
			a++;
			if(n==4){
				n=1;
				m=0;
				document.querySelector(".rIcons ul").display.left="0";
			}
		}
	},100);
}
...
//放大镜
window.onload=function(){
	var Bigimage1=document.querySelector("#img1 .Bigimage");
	var block=$("block");
	var extend=$("extend");
	var iright=$("iright");
	//放大倍数
	var scale=2;
	var lw=Bigimage1.offsetWidth;
	var lh=Bigimage1.offsetHeight;

	block.style.width=lw/scale+"px";
	block.style.height=lh/scale+"px";

	iright.style.width=lw*scale +"px";
	iright.style.height=lh*scale +"px";

	Bigimage1.onmouseover=function(){
		block.style.display="block";
		extend.style.display="block";
	}

	Bigimage1.onmouseout=function(){
		block.style.display="none";
		extend.style.display="none";
	}

	Bigimage1.onmousemove=function(e){
		e=e || event;
		var x=e.clientX-block.offsetWidth;
		var y=e.clientY-block.offsetHeight;
		if(x<=0){
			x=0;
		}
		if(y<=0){
		...
		if(x>=lw-block.offsetWidth){
			x=lw-block.offsetWidth;
		}
		if(y>=lh-block.offsetHeight){
			y=lh-block.offsetHeight;
		}
		block.style.left=x+"px";
		block.style.top=y+"px";
		iright.style.left=-x*scale+"px";
		iright.style.top=-y*scale+"px";
	}
